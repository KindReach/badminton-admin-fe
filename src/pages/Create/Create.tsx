...
const PublishCheck = ({ show, setShow, sessions }: Props) => {
  ...
  return (
    ...
    >
      ...
      >
        <Offcanvas.Title>
          <button className={styles.submitBtn} onClick={handleSubmit}>確認發佈</button>
        </Offcanvas.Title>
      </Offcanvas.Header>
      <Offcanvas.Body>
        <div className={styles.checkContainer}>
          ...
          { sessions.map((item, index) => {
            ...
            return <div className={styles.session} key={index} >
                <h1>{ item.place_name }</h1>
                <div className={styles.description} >
                  <p><IoCalendarClearOutline style={{ marginRight: "5px" }} />{item.date}</p>
                  <p><FaRegClock style={{ marginRight: "5px" }}  />{item.start_time}~{item.end_time}</p>
                  <p><GoPeople style={{ marginRight: "5px" }} />上限 {item.limit_of_member} 人</p>
                </div>
...
const Create = () => {
  ...
  const singleState = useSelector((state: RootState) => state.publish.single_state);
