          ...
          <div className={styles.inputGroup}>
            <div className="d-flex justify-content-between align-items-center">
              ...
              <button 
                type="button"
                onClick={resetRegion}
                className={`btn btn-link p-0 ${!selectedRegion ? 'd-none' : ''}`}
              ...
              {showRegionFilter && (
                <ul className="position-absolute w-100 mt-1 list-group shadow-sm" style={{ zIndex: 1000 }}>
                  {regions.map(region => (
                    <li 
                      key={region} 
                      onClick={() => { selectRegion(region); setErrorOfRegion(false); }}
                      className={`list-group-item ${selectedRegion === region ? 'active' : ''}`}
                      style={{ cursor: 'pointer' }}
                    >
                      {region}
                    </li>
            ...
            <div className="position-relative">
              ...
              {showSuggestions && filteredVenues.length === 0 && placeName && (
                <div className="position-absolute w-100 p-2 text-center bg-light border rounded">
                  ...
                  {selectedRegion && (
                    <div><small className="text-muted">已篩選區域: {selectedRegion}</small></div>
                  )}
                </div>
              )}
            </div>
